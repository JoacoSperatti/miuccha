...
// --- CONFIGURACIÓN DE ALERTAS TOAST (Estilo notificación flotante) ---
    ...
    <div className={`fixed inset-0 z-[200] ${isOpen ? "visible" : "invisible"}`}>
      ...
      <div className={`absolute right-0 top-0 h-full bg-white w-full max-w-md shadow-2xl transition-transform duration-500 transform ${isOpen ? "translate-x-0" : "translate-x-full"} flex flex-col`}>
        ...
        <div className="flex-grow overflow-y-auto p-6 space-y-6 font-sans">
          {cart.length === 0 ? (
            <p className="text-center text-gray-400 mt-20 text-[10px] uppercase tracking-widest font-bold">Carrito vacío</p>
          ) : (
            cart.map((item, index) => (
        ...
        {cart.length > 0 && (
          <div className="p-8 bg-gray-50 space-y-4 border-t-2 border-white">
            <div className="bg-amber-50 border border-amber-200 p-4">
              <p className="text-[9px] font-bold text-amber-800 uppercase tracking-widest mb-1 font-sans">📌 Pago por Transferencia: pagos.miuccha</p>
  ...
  return (
    <div className="group flex flex-col items-center text-center p-4">
      <div className="relative overflow-hidden w-full aspect-[3/4] mb-4 bg-gray-100 shadow-sm group">
        <img src={images[imgIndex]} alt={product.nombre} className="w-full h-full object-cover transition-transform group-hover:scale-105 duration-700" />
        {images.length > 1 && (
      ...
      {hasColors && (
        <div className="flex flex-wrap justify-center gap-1 mb-3 font-sans">
          {coloresArray.map((c) => (
            <button key={c} onClick={() => setSelectedColor(c)} className={`px-3 py-1 text-[9px] uppercase tracking-widest border transition-all ${selectedColor === c ? "bg-black text-white border-black" : "bg-white text-gray-600 border-gray-200 hover:border-black"}`}>{c}</button>
          ))}
        </div>
      )}

      <button onClick={onOpenSizeGuide} className="text-[9px] uppercase tracking-widest text-gray-400 mb-2 border-b border-transparent hover:border-gray-400 transition-all font-sans font-bold">Ver Guía de Talles</button>

      <div className="flex flex-wrap justify-center gap-2 mb-4 font-sans">
        {stockEntries.map(([talle, cant]) => (
          <button key={talle} disabled={cant <= 0} onClick={() => setSelectedSize(talle)} className={`w-8 h-8 text-[9px] border flex items-center justify-center transition-all ${cant <= 0 ? "opacity-20 cursor-not-allowed bg-gray-50 line-through" : "hover:border-black font-bold"} ${selectedSize === talle ? "bg-black text-white border-black" : "bg-white text-black border-gray-200"}`}>{talle}</button>
        ))}
      </div>

      <button onClick={handleAddToCart} disabled={!canAdd} className={`w-full py-3 text-[10px] uppercase tracking-widest font-bold transition-all font-sans ${canAdd ? "bg-black text-white hover:bg-gray-800 shadow-lg" : "bg-gray-100 text-gray-400 cursor-not-allowed"}`}>
    ...
    <div className="fixed top-0 w-full z-50">
      ...
      <nav className={`bg-white/95 backdrop-blur-md border-b border-gray-100 px-6 flex justify-between items-center transition-all duration-500 ${isScrolled ? "py-2" : "py-6"}`}>
        <div className="w-1/3 flex items-center gap-6 font-sans text-[10px] uppercase tracking-[0.2em] text-gray-400 font-bold">
          ...
          <a href="#informacion" className="hover:text-black hidden md:block transition">Información</a>
          <button onClick={() => setIsMenuOpen(true)} className="md:hidden text-black p-2"><FaBars size={20} /></button>
        </div>
...
const CategoryGrid = () => {
  ...
  return (
    <section className="grid grid-cols-1 md:grid-cols-2 w-full shadow-2xl border-t-2 border-white">
      ...
      <div onClick={() => { navigate("/catalogo"); window.scrollTo(0, 0); }} className="flex flex-col items-center justify-center p-10 bg-[#C37D8D] border-2 border-white h-[250px] md:h-[300px] text-center font-serif italic cursor-pointer hover:opacity-90 transition-opacity">
        <h4 className="text-4xl md:text-6xl text-[#E3F285] uppercase tracking-tighter">MUCHO MÁS</h4>
      </div>
    </section>
  );
};
...
const HomeHero = () => {
  ...
  return (
    <section className="relative w-full h-[75vh] md:h-[80vh] overflow-hidden z-0">
      <Swiper modules={[Autoplay, Pagination, Navigation]} autoplay={{ delay: 5000 }} pagination={{ clickable: true }} navigation={true} loop={true} className="h-full w-full">
        {slides.map((s) => (
      ...
      <section className="py-24 px-6 max-w-7xl mx-auto text-center font-sans">
        ...
        {loading ? <p className="font-serif italic text-gray-400">Cargando colección...</p> : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-12">
            {featured.map((p) => <ProductCard key={p.id} product={p} onAddToCart={onAddToCart} onOpenSizeGuide={onOpenSizeGuide} />)}
          </div>
        )}
      </section>
    </div>
  ...
  return (
    <div className="pt-40 min-h-screen max-w-7xl mx-auto px-6 font-sans">
      ...
      <div className="flex flex-wrap justify-center gap-4 mb-20">
        {cats.map((c) => (
          <button key={c} onClick={() => setSearchParams(c === "TODO" ? {} : { cat: c })} className={`px-8 py-2 text-[10px] uppercase tracking-[0.2em] font-bold border transition-all ${(c === "TODO" && !categoryFilter) || categoryFilter === c ? "bg-black text-white shadow-lg" : "bg-white text-black border-gray-200 hover:border-black"}`}>
            {c}
          </button>
        ))}
      </div>
      {loading ? <p className="text-center italic font-serif text-gray-400">Actualizando productos...</p> : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-12 mb-24">
          {products.map((p) => <ProductCard key={p.id} product={p} onAddToCart={onAddToCart} onOpenSizeGuide={onOpenSizeGuide} />)}
        </div>
      )}
    </div>
  );
...
const AdminPanel = () => {
  ...
  const handleLogin = (e) => {
    ...
    if (passwordInput === "miuccha2026") {
      ...
      Swal.fire({ icon: 'error', title: 'Acceso Denegado', text: 'Contraseña incorrecta', confirmButtonColor: '#000' });
      setPasswordInput("");
    }
  };

  // Compresor de imagen automático
  ...
  const handleDeleteImage = async (id, type, index = null) => {
    ...
    try {
      ...
      console.error(e);
      Swal.fire({ icon: 'error', title: 'Error', text: 'No se pudo eliminar la imagen' });
    }
  };
  ...
  const handleCreate = async (e) => {
    e.preventDefault();
    if (!newProd.img) {
      Swal.fire({ icon: 'warning', title: 'Falta la imagen principal', text: 'Por favor seleccioná una portada para el producto.', confirmButtonColor: '#000' });
      return;
    }
    try {
      ...
      Swal.fire({ icon: 'success', title: '¡Excelente!', text: 'El producto fue creado con éxito.', confirmButtonColor: '#000' });
      setNewProd({
        nombre: "", precio: "", categoria: "BOTAS", destacado: false, img: "", colores: "", galeria: [],
        stock: { 35: 0, 36: 0, 37: 0, 38: 0, 39: 0, 40: 0 },
      });
      ...
      console.error(e); 
      Swal.fire({ icon: 'error', title: 'Error', text: 'Hubo un problema al crear el producto.' });
    }
  };
  ...
  const updateField = async (id, field, value) => {
    try {
      ...
      fetchProducts();
      if(field !== 'img' && field !== 'galeria') Toast.fire({ icon: 'success', title: 'Campo actualizado' });
  ...
  const deleteProduct = async (id) => {
    ...
    if (result.isConfirmed) {
      ...
      fetchProducts();
      Swal.fire({ title: '¡Borrado!', text: 'El producto ha sido eliminado.', icon: 'success', confirmButtonColor: '#000' });
    }
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 px-6 font-sans">
        <div className="bg-white p-10 rounded-xl shadow-2xl max-w-md w-full space-y-8 text-center border-t-4 border-black">
          <div>
            <h2 className="font-serif text-4xl italic tracking-widest text-gray-900 mb-2">MIUCCHA</h2>
            <p className="text-[10px] uppercase tracking-[0.3em] font-bold text-gray-400">Acceso Restringido</p>
          </div>
          <form onSubmit={handleLogin} className="space-y-4">
            <input 
              ...
              autoFocus
            />
            <button type="submit" className="w-full bg-black text-white py-4 uppercase text-[10px] tracking-[0.3em] font-bold hover:bg-gray-800 transition-colors">
              Ingresar al Panel
            </button>
          </form>
          <Link to="/" className="block text-[10px] text-gray-400 hover:text-black uppercase tracking-widest mt-6">Volver a la tienda</Link>
        </div>
      </div>
    );
  }

  if (loading) return <div className="pt-40 text-center font-serif italic text-gray-400">Cargando catálogo...</div>;
  ...
  return (
    <div className="pt-40 px-6 max-w-6xl mx-auto mb-20 font-sans">
      <div className="flex justify-between items-end border-b pb-4 mb-8">
        <h2 className="font-serif text-3xl italic tracking-widest uppercase">Panel de Control</h2>
        <span className="text-[10px] font-bold text-green-600 bg-green-50 px-3 py-1 rounded-full uppercase tracking-widest">En Línea</span>
      </div>

      {/* FORMULARIO NUEVO PRODUCTO */}
      <form onSubmit={handleCreate} className="bg-white p-8 rounded-xl shadow-lg mb-16 border grid grid-cols-1 md:grid-cols-3 gap-6">
        <h3 className="col-span-full font-serif text-2xl mb-2">Cargar Nuevo Modelo</h3>
        
        <div className="flex flex-col gap-1">
          <label className="text-[9px] uppercase tracking-widest font-bold text-gray-500">Nombre</label>
          <input type="text" className="p-3 border text-sm focus:outline-none focus:border-black" value={newProd.nombre} onChange={(e) => setNewProd({ ...newProd, nombre: e.target.value })} required />
        </div>
        
        <div className="flex flex-col gap-1">
          <label className="text-[9px] uppercase tracking-widest font-bold text-gray-500">Precio</label>
          <input type="number" className="p-3 border text-sm focus:outline-none focus:border-black" value={newProd.precio} onChange={(e) => setNewProd({ ...newProd, precio: e.target.value })} required />
        </div>
        
        <div className="flex flex-col gap-1">
          <label className="text-[9px] uppercase tracking-widest font-bold text-gray-500">Categoría</label>
          <select className="p-3 border text-sm focus:outline-none focus:border-black" value={newProd.categoria} onChange={(e) => setNewProd({ ...newProd, categoria: e.target.value })}>
        ...
        <div className="flex flex-col gap-1">
          <label className="text-[9px] uppercase tracking-widest font-bold text-gray-500">Colores Disponibles (Opcional)</label>
          <input type="text" placeholder="Ej: Negro, Suela, Blanco" className="p-3 border text-sm focus:outline-none focus:border-black" value={newProd.colores} onChange={(e) => setNewProd({ ...newProd, colores: e.target.value })} />
        </div>
        
        <div className="md:col-span-2 flex flex-col gap-2 p-4 border bg-gray-50 rounded">
          <span className="text-[10px] font-bold text-gray-600 uppercase tracking-widest border-b pb-2 mb-2">Imágenes del producto</span>
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1">
              <label className="text-[9px] uppercase font-bold text-gray-400 mb-1 block">Foto Principal (Portada)</label>
              <input type="file" accept="image/*" className="text-xs w-full bg-white p-2 border" onChange={(e) => handleSingleFile(e, (base64) => setNewProd({...newProd, img: base64}))} />
            </div>
            <div className="flex-1">
              <label className="text-[9px] uppercase font-bold text-gray-400 mb-1 block">Fotos Extras (Galería)</label>
              <input type="file" multiple accept="image/*" className="text-xs w-full bg-white p-2 border" onChange={(e) => handleMultipleFiles(e, (base64Array) => setNewProd({...newProd, galeria: [...newProd.galeria, ...base64Array]}))} />
            </div>
          </div>
        </div>
        ...
        {(newProd.img || newProd.galeria.length > 0) && (
          <div className="col-span-full p-4 border bg-gray-50 rounded flex flex-wrap gap-4 items-center">
            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest w-full border-b pb-2 mb-2">Previsualización</span>
            {newProd.img && (
              <div className="relative w-20 h-28 group">
                <img src={newProd.img} className="w-full h-full object-cover border-2 border-black" alt="Portada" />
                <span className="absolute bottom-0 inset-x-0 bg-black/80 text-white text-[7px] text-center font-bold py-1">PORTADA</span>
                <button type="button" onClick={() => setNewProd({...newProd, img: ""})} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full w-5 h-5 flex items-center justify-center text-[10px] shadow-lg hover:scale-110">✕</button>
              </div>
            )}
            {newProd.galeria.map((img, idx) => (
              <div key={idx} className="relative w-20 h-28 group">
                ...
                }} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full w-5 h-5 flex items-center justify-center text-[10px] shadow-lg hover:scale-110">✕</button>
              </div>
            ))}
          </div>
        )}
        ...
        <div className="col-span-full flex flex-col gap-2 mt-4">
          <span className="text-[10px] font-bold text-gray-600 uppercase tracking-widest border-b pb-2 mb-2">Stock Inicial por Talle</span>
          <div className="grid grid-cols-4 sm:grid-cols-9 gap-2">
            {Object.keys(newProd.stock).map((talle) => (
              <div key={talle} className="flex flex-col gap-1 items-center bg-gray-50 p-2 border rounded">
                <span className="text-[10px] font-bold text-gray-500">T.{talle}</span>
                <input type="number" className="w-full p-2 border text-center text-xs focus:border-black focus:outline-none" value={newProd.stock[talle]} onChange={(e) => setNewProd({ ...newProd, stock: { ...newProd.stock, [talle]: e.target.value } })} />
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="col-span-full flex items-center justify-between mt-6 pt-6 border-t">
          <label className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest cursor-pointer">
            <input type="checkbox" className="w-4 h-4 accent-black" checked={newProd.destacado} onChange={(e) => setNewProd({ ...newProd, destacado: e.target.checked })} /> 
            Destacar en Inicio
          </label>
          <button type="submit" disabled={uploading} className="bg-black text-white px-10 py-4 uppercase text-[10px] tracking-[0.2em] font-bold disabled:bg-gray-400 hover:bg-gray-800 transition-colors shadow-lg">
      ...
      <div className="space-y-6">
        {products.map((p) => (
          <div key={p.id} className="p-6 border bg-white shadow-sm flex flex-col md:flex-row gap-6 relative rounded-lg">
            <button onClick={() => deleteProduct(p.id)} className="absolute top-4 right-4 text-red-500 hover:text-red-700 hover:scale-110 transition-transform p-2"><FaTrash size={16} /></button>
            
            <div className="w-full md:w-1/3 flex flex-col gap-4">
              <div className="flex flex-wrap gap-2 p-3 border bg-gray-50 rounded min-h-[120px]">
                <div className="relative w-20 h-28 group">
                  <img src={p.img} className="w-full h-full object-cover border-2 border-black" alt="Portada"/>
                  <span className="absolute bottom-0 inset-x-0 bg-black/80 text-white text-[7px] text-center font-bold py-1">PORTADA</span>
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <label htmlFor={`main-${p.id}`} className="text-[8px] border border-gray-300 py-3 text-center cursor-pointer hover:bg-black hover:text-white font-bold uppercase transition-all rounded">
                ...
                <label htmlFor={`gal-${p.id}`} className="text-[8px] bg-black text-white py-3 text-center cursor-pointer hover:bg-gray-800 font-bold uppercase transition-all rounded shadow">
            ...
            <div className="flex-grow grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div className="space-y-4">
                <div>
                  <label className="text-[8px] uppercase tracking-widest font-bold text-gray-400">Nombre</label>
                  <input type="text" defaultValue={p.nombre} onBlur={(e) => updateField(p.id, "nombre", e.target.value)} className="w-full font-bold border-b pb-1 focus:outline-none focus:border-black text-lg" />
                </div>
                <div>
                  <label className="text-[8px] uppercase tracking-widest font-bold text-gray-400">Colores (separar con coma)</label>
                  <input type="text" placeholder="Agregar colores..." defaultValue={(p.colores || []).join(", ")} onBlur={(e) => updateField(p.id, "colores", e.target.value.split(",").map(c=>c.trim()).filter(c=>c!==""))} className="w-full border-b pb-1 focus:outline-none focus:border-black text-sm text-gray-600" />
                </div>
                <div className="flex items-end gap-6 pt-2">
                  <div>
                    <label className="text-[8px] uppercase tracking-widest font-bold text-gray-400">Precio ($)</label>
                    <input type="number" defaultValue={p.precio} onBlur={(e) => updateField(p.id, "precio", e.target.value)} className="w-24 border-b pb-1 focus:outline-none focus:border-black text-lg font-bold" />
                  </div>
                  <label className="text-[10px] flex items-center gap-2 font-bold uppercase cursor-pointer mb-1 border px-3 py-1 rounded bg-gray-50">
                    <input type="checkbox" className="accent-black" defaultChecked={p.destacado} onChange={(e) => updateField(p.id, "destacado", e.target.checked)} /> 
                    Destacado Home
                  </label>
                </div>
              </div>
              ...
              <div className="bg-gray-50 p-4 border rounded">
                <label className="text-[9px] uppercase tracking-widest font-bold text-gray-500 block mb-3 text-center">Control de Stock</label>
                <div className="grid grid-cols-3 gap-2">
                  {Object.entries(p.stock || {}).map(([talle, cant]) => (
                    <div key={talle} className="flex flex-col items-center bg-white border p-1 rounded">
                      <span className="text-[9px] font-bold text-gray-400">T.{talle}</span>
                      <input type="number" defaultValue={cant} onBlur={(e) => updateStock(p.id, talle, e.target.value)} className="w-10 text-center text-sm font-bold bg-transparent outline-none focus:text-blue-600" />
                    </div>
                  ))}
                </div>
              </div>

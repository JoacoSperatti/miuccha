  ...
  return (
    <div className="group flex flex-col items-center text-center p-4">
      ...
      {showModal && (
        ...
        >
          <div 
            className="max-w-5xl w-full animate-in fade-in zoom-in duration-300 relative"
            onClick={(e) => e.stopPropagation()}
          >
            <button 
              className="absolute -top-12 right-0 text-white text-2xl hover:scale-110 transition-transform z-10 p-2"
              onClick={() => setShowModal(false)}
            >
              ✕
            </button>
      ...
      >
        <button 
          onClick={handleShare}
          className="absolute top-4 right-4 z-30 bg-white/80 hover:bg-white p-2 rounded-full shadow-md text-black transition-all hover:scale-110 md:opacity-0 md:group-hover:opacity-100"
          title="Compartir"
        >
          <FiShare2 size={16} />
        </button>

        <div className="relative overflow-hidden w-full aspect-[3/4] mb-4 bg-gray-100 shadow-sm group-hover:shadow-md transition-shadow">
          {images.length > 0 ? (
            <img
              src={images[imgIndex]}
              alt={product.nombre}
